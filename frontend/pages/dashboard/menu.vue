      ...
      <v-container>
        ...
        <v-row>
            ...
            <v-card>
              ...
              <v-card-text>
                ...
                <v-row v-if="bill_items.length > 0">
                  <v-col cols="12">
                    <v-btn block color="primary" @click="confirm(bill_items)">
          ...
          <v-col cols="12" v-if="order">
            ...
            <v-row>
              <v-col
                v-for="item in getItems(searchVal)"
                :key="item.id"
                cols="12"
                md="3"
              >
...
export default {
  ...
  methods: {
    ...
    getItems(searchVal) {
      if (searchVal) {
        let list = this.menu.filter((i) => i.name.includes(searchVal));
    ...
    async confirm(bill_items) {
      let order = await this.$store.dispatch("order/store", {
  ...
  watch: {
    items() {
      this.menu = JSON.parse(JSON.stringify(this.items));
    },
  },
};
</script>
