export default (context, inject) => {
  ...
  inject('save', (item, resource, redirect = null, file_attribute = null) => {
    let file = null;
    if (file_attribute) {
    ...
    redirect = null
    if (item.id) {
      return context.store.dispatch(`${resource}/update`, item).then((data) => {
        context.$upload(file, resource, data);
      });
    } else {
      return context.store.dispatch(`${resource}/store`, item).then((data) => {
        context.$upload(file, resource, data);
